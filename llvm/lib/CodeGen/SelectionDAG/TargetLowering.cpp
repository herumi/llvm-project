...
SDValue TargetLowering::BuildUDIV(SDNode *N, SelectionDAG &DAG,
  ...
  auto BuildUDIVPattern = [&](ConstantSDNode *C) {
    ...
    if (Divisor.isOne()) {
      ...
      // Our Approach: For 32-bit division on 64-bit targets with IsAdd (33-bit
      ...
      bool Is64BitTarget = DAG.getDataLayout().getPointerSizeInBits() >= 64;
      if (EltBits == 32 && !VT.isVector() && Is64BitTarget && magics.IsAdd &&
          magics.PreShift == 0) {
        // For IsAdd case, actual magic constant is 2^32 + Magic (33-bit)
        unsigned OriginalShift = magics.PostShift + 33; // P in the algorithm
        APInt RealMagic = magics.Magic.zext(65);
        RealMagic = APInt(65, 1).shl(32) + RealMagic; // 2^32 + Magic
